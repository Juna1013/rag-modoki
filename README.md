...
埋め込みベクトルによる意味検索を用いた RAG パイプラインを実装しています。展示当初はキーワードマッチングによる簡易検索（旧称「rag-modoki」の由来）でしたが、現在は次の構成です。
...
② 拡張 (Augmentation)
   取得したチャンクを出典番号付きで参考資料としてプロンプトに埋め込む
                    ...
                    ▼
③ 生成 (Generation)
   Gemini (gemini-2.5-flash) が参考資料を根拠に回答を生成
                    │
                    ▼
...
| AI | Gemini API（生成: gemini-2.5-flash / 埋め込み: gemini-embedding-001） |
